...
module.exports = class Entreprise extends User {
    ...
    update = (Id_entrepold) => {
        return new Promise((resolve, reject) => {
            this.dbconnection.query(`UPDATE ${this.tablename} SET Raison_social = ?, Secteur_activite = ?, Site_web = ?, Localite = ?, Description = ? where Id_utilisateur= ${Id_entrepold}`, [this.Raison_social, this.Secteur_activite, this.Site_web, this.Localite, this.Description], (error, results, fields) => {
                if (error) return reject(error)
                resolve(results)
            })
        })
    ...
    static delet(Id_entrepold) {
        return new Promise((resolve, reject) => {
            this.dbconnection.query(`DELETE FROM ${this.tablename} where Id_utilisateur = ${Id_entrepold}`, (error, results, fields) => {
                if (error) return reject(error)
                resolve(results)
            })
        })
