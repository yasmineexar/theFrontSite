...
module.exports = class Metier extends Model {
  ...
  update = () => {
    return new Promise((resolve, reject) => {
      this.dbconnection.query(`UPDATE ${this.tablename} SET Description = ?, Nom = ?, Id_faculte = ? where Id_metier = ${this.Id_metier}`, [this.Description, this.Nom, this.Id_faculte], (error, results, fields) => {
        if (error) return reject(error)
        resolve(results)
      })
    })
