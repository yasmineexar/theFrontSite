...
module.exports = class Offre extends Model {
  ...
  static getByIdUser(id){
    return new Promise((resolve, reject) => {
      this.dbconnection.query(`SELECT * from ${this.tablename} where Id_utilisateur = ?`,[id], (error, results, fields) => {
  ...
  creat = () => {
    return new Promise((resolve, reject) => {
      this.dbconnection.query(`INSERT INTO ${this.tablename} SET Duree_stage = ?, Base_remuneration = ?, Nb_places = ?, Description = ?, Titre = ?, Id_faculte = ?,Date_publication = NOW(),Id_utilisateur = ?`, [this.Duree_stage, this.Base_remuneration, this.Nb_places, this.Description, this.Titre, this.Id_faculte,this.Id_utilisateur], (error, results, fields) => {
        if (error) return reject(error)
        resolve(results)
      })
    })
