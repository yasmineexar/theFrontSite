...
module.exports = class Offre extends Model {
  ...
  creat = () => {
    return new Promise((resolve, reject) => {
      this.dbconnection.query(`INSERT INTO ${this.tablename} SET Duree_stage = ?, Base_remuneration = ?, Nb_places = ?, Description = ?, Titre = ?`, [this.Duree_stage, this.Base_remuneration, this.Nb_places, this.Description, this.Titre], (error, results, fields) => {
        if (error) return reject(error)
        resolve(results)
      })
    })
